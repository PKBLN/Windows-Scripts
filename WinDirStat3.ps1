...
<#
.SYNOPSIS
    WPF-Speicherplatz-Analyzer mit interaktiver SequoiaView-artiger Treemap.
...
.DESCRIPTION
    ...
    - Squarified-Treemap für die globalen Top-Dateien oder den gewählten Ordner
    - Farben nach Dateityp, Tooltips, synchronisierte Auswahl und Öffnen per Doppelklick
    - Einstellbares Rechtecklimit und Sammelfläche für eine flüssige Darstellung
    ...
    Treemap-Datenbasis und Grenzen:
    Die Treemap verwendet dieselben Daten wie die Dateiliste: entweder die globalen Top 200
    des letzten Scans oder alle direkt im ausgewählten Ordner liegenden Dateien. Sie führt
    keinen zusätzlichen rekursiven Scan aus. Das wählbare Limit (Standard: 500 Rechtecke)
    begrenzt nur die WPF-Darstellung; bei mehr Dateien fasst ein proportionales Rechteck den
    Rest zusammen. 0-Byte-Dateien besitzen keine sichtbare Fläche. Angezeigt werden logische
    Dateigrößen; NTFS-Komprimierung, Sparse Files und belegter Speicher können davon abweichen.
#>
...
if ([Threading.Thread]::CurrentThread.ApartmentState -ne [Threading.ApartmentState]::STA) {
    ...
    throw 'Dieses Skript muss in einem STA-Thread laufen. Starte es z. B. mit powershell.exe -STA -File .\WinDirStat3.ps1'
}
...
<Window
    ...
    Title="WinDirStat 3.0 - PowerShell Speicheranalyse mit Treemap"
    Width="1250"
    Height="760"
    MinWidth="900"
    MinHeight="520"
                            ...
                            <Grid Margin="2">
                                ...
                                <DockPanel Grid.Row="0" Margin="5,3,5,6" LastChildFill="True">
                                    ...
                                    <TextBlock x:Name="TreemapInfoText"
                                               Foreground="#666666"
                                               VerticalAlignment="Center"
                                               Text="Nach einem Scan steht die Treemap hier bereit."
                                               TextTrimming="CharacterEllipsis"/>
                                </DockPanel>
                                ...
                                <Border Grid.Row="1" BorderBrush="#50555B" BorderThickness="1"
                                    ...
                                    <Grid x:Name="TreemapHost" Background="#171A1E"
                                        ...
                                        <TextBlock x:Name="TreemapEmptyText"
                                                   Text="Keine darstellbaren Dateien."
...
function Get-TreemapDisplayRows {
    ...
    # Ein Platz bleibt für die Sammelfläche frei. So bleibt die Gesamtfläche
    # proportional, ohne für sehr große Ordner tausende WPF-Elemente anzulegen.
    ...
    $displayRows.Add([PSCustomObject]@{
        Name          = ('Weitere {0:N0} Dateien' -f $remainingCount)
...
function Add-TreemapStrip {
    ...
    if ($Bounds.Width -ge $Bounds.Height) {
        # Breiter Restbereich: links eine senkrechte Spalte ablegen.
        ...
        # Hoher Restbereich: oben eine waagerechte Zeile ablegen.
...
function Get-TreemapBrush {
    ...
    # Der diagonale Verlauf imitiert die plastischen "Cushions" von SequoiaView,
    # bleibt aber deutlich günstiger als ein eigener Pixel-Renderer.
...
function Open-TreemapFile {
    ...
    try {
        if ([System.IO.File]::Exists($Path)) {
            ...
            $StatusText.Text = 'Die ausgewählte Datei ist nicht mehr vorhanden.'
        }
    }
    catch {
...
function Show-Treemap {
    ...
    if ($width -lt 20 -or $height -lt 20) {
        $TreemapEmptyText.Text = 'Treemap wird vorbereitet…'
    ...
    if ($positiveCount -eq 0 -or $totalBytes -le 0) {
        $TreemapInfoText.Text = 'Keine Dateien mit einer Größe größer als 0 Byte.'
        $TreemapEmptyText.Text = 'Keine darstellbaren Dateien.'
        $TreemapEmptyText.Visibility = [System.Windows.Visibility]::Visible
        return
    }
    ...
    if ($positiveCount -gt $limit) {
        $TreemapInfoText.Text = (
            '{0:N0} Dateien • {1} • {2:N0} größte einzeln, Rest zusammengefasst' -f
                $positiveCount, (Format-Size $totalBytes), $individualCount
        )
    }
    else {
        $TreemapInfoText.Text = (
            '{0:N0} Dateien • {1} • Fläche entspricht Dateigröße' -f
    ...
    foreach ($rectangle in $layout) {
        ...
        if ($isAggregate) {
            $border.ToolTip = (
                "{0}`n{1}`nSammelfläche zur Begrenzung der Darstellung" -f
...
function Start-Scan {
    ...
    $script:TreemapElementsByPath = @{}
    $script:SelectedTreemapElement = $null
    $TreemapInfoText.Text = 'Scan läuft…'
    $TreemapEmptyText.Text = 'Treemap wird nach dem Scan aufgebaut.'
    $TreemapEmptyText.Visibility = [System.Windows.Visibility]::Visible
    $script:GlobalTopFiles = @()
    $script:CurrentFilesMode = 'Top'
    $script:CurrentFolderPath = $null
...
$FileViews.Add_SelectionChanged({
    ...
    # SelectionChanged ist ein RoutedEvent und erreicht den TabControl auch aus
    # DataGrid/ComboBox. Nur einen echten Tabwechsel behandeln.
