...
if ([Threading.Thread]::CurrentThread.ApartmentState -ne [Threading.ApartmentState]::STA) {
    ...
    throw 'This script must run in an STA thread. For example, start it with powershell.exe -STA -File .\WinDirStat.en.ps1'
}
...
<Window
    ...
    Title="StorageTree 2.1 - PowerShell Storage Analysis"
    Width="1250"
    Height="760"
    MinWidth="900"
    MinHeight="520"
...
function Complete-Scan {
    try {
        ...
        if ($null -ne $root) {
            ...
            Show-FileRows -Rows $topRows `
                -Heading ('Top {0:N0} largest files' -f $topRows.Count) `
